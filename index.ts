  ...
  while (true) {
    try {
      ...
      if (currentGas > CONFIG.maxGasLamports) {
        console.warn(`⚠️  Gas spike (${currentGas} lamports). Sleeping...`);
...
async function runCommitCycle(
  ...
  for (const query of queries) {
    ...
    if (saltCache[queryKey]) {
      console.log(`  ⏭️  Already committed: ${data.uniqueEventId}`);
      continue;
    }
...
async function runRevealCycle(
  ...
  for (const query of queries) {
    ...
    if (now > data.revealDeadline.toNumber()) {
      console.log(`  ⚠️  Reveal deadline passed: ${data.uniqueEventId}`);
      // Clean up cache
    ...
    try {
      ...
      if (e.message?.includes("AlreadyRevealed")) {
        console.log(`  ⏭️  Already revealed`);
        delete saltCache[queryKey];
      } else {
